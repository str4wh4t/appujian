...
class Master_model extends CI_Model {
    ...
    {
        $this->datatables->select('a.id_mahasiswa, a.nama, a.nim, a.email, GROUP_CONCAT(c.nama_matkul SEPARATOR "---") as nama_matkul, (SELECT COUNT(id) FROM users WHERE username = a.nim) AS ada');
        $this->datatables->from('mahasiswa a');
        $this->datatables->join('mahasiswa_matkul b', 'b.mahasiswa_id=a.id_mahasiswa');
...
//        $this->db->join('jurusan', 'jurusan_id=id_jurusan');
    ...
    {
        $this->datatables->select('a.id_dosen,a.nip, a.nama_dosen, a.email, a.matkul_id, GROUP_CONCAT(c.nama_matkul SEPARATOR "---") as nama_matkul, (SELECT COUNT(id) FROM users WHERE username = a.nip OR email = a.email) AS ada');
        $this->datatables->from('dosen a');
        $this->datatables->join('dosen_matkul b', 'a.id_dosen = b.dosen_id');
        $this->datatables->join('matkul c', 'b.matkul_id = c.id_matkul');
        $this->datatables->group_by('a.id_dosen');
        return $this->datatables->generate();
    }
    ...
    public function getAllTopik()
    {
        ...
        $this->db->from('topik AS a');
        $this->db->join('matkul AS b', 'a.matkul_id = b.id_matkul');
        $this->db->order_by('a.nama_topik');
