...
function init_page_level(){
  ...
  table = $("#bundle_soal").DataTable({
    ...
    serverSide: true,
    ajax: {
      url: base_url + "soal/ajax/data_bundle_soal",
      type: "POST"
    },
    columns: [
      { data: "nama_bundle" },
    ...
    columnDefs: [
      {
        ...
        render: function(data, type, row, meta) {
          return `<div class="btn-group">
                  <a class="btn btn-sm btn-info" href="${base_url}soal/index#${data}---${row['nama_bundle']}">
										<i class="fa fa-folder-open"></i>
									</a>
                  <a class="btn btn-sm btn-warning btn_edit" href="${base_url}soal/edit_bundle_soal/${data}">
...
$(document).on("click", ".btn_delete", function() {
    ...
    if (confirm('Yakin akan dihapus ?')){
      ...
      $.ajax({
        type: "POST",
        url: "{{ url('soal/ajax/delete_bundle_soal') }}",
        data: {
            'id': id,
        },
        success: function (r) {
